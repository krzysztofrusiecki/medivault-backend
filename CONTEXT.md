...
**LAB_ADMIN**:
A user role representing staff at exactly one registered `Lab` (one lab per admin, not many-to-many). Attached to a `Lab` by a `SUPER_ADMIN` — cannot self-register a `Lab`.
